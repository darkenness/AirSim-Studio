...
- 链式墙绘制 + 原子矩形创建（一步撤销）
- 16 种气流路径元件：门、窗、开口、裂缝、风机、风管、风阀、基础过滤器、自调节通风口、单向阀、二次元件、防回流阻尼器、供应扩散器、回风栅栏、气体过滤器、UVGI 紫外杀菌过滤器
- 稳态气流求解：Newton-Raphson + 信赖域/亚松弛
- 瞬态污染物传输：隐式欧拉时间积分 + 自适应步长 (BDF-1/BDF-2)
- 5 种污染源类型：恒定、指数衰减、压力驱动、浓度截止、爆发式释放
- 控制系统：传感器 → PI 控制器（死区+抗饱和）→ 执行器 + 15 种逻辑节点
- 人员暴露评估：累积吸入剂量 + 峰值浓度跟踪
- 多楼层支持：独立几何 + 竖井跨楼层自动连通 + 楼层复制
- 结果可视化：流量箭头、压力标签、浓度热力图、风压矢量
- 瞬态结果回放：TimeStepper 播放控制 + 多倍速
- 底图追踪：导入图片 + 两点校准 + 比例尺设置
- 暗色模式（偏好持久化）
- 多种报告输出：ACH、CSM、CBW、VAL、EBW、CEX、LOG
...
| ↑ | 切换到上一楼层（快捷键 `PageUp`） |
| 楼层编号 | 点击切换到该层 |
| ↓ | 切换到下一楼层（快捷键 `PageDown`） |
| + | 添加新楼层（空白画布） |
| 复制 | 复制当前楼层的几何和组件 |
| 删除 | 删除当前楼层（仅剩一层时禁用） |
| ⊡ | 缩放适配：自动缩放画布以显示当前楼层全部几何（60px 边距） |
...
也可为每层设置绝对标高（`Story.elevation`），覆盖自动计算值，适用于混合层高的建筑。

标高差异驱动烟囱效应（热压）计算。

### 5.4 竖井与跨楼层连通

多楼层建筑中，竖井（电梯井、楼梯间）需要跨楼层连通。

**配置方式**：选中区域 → 在属性面板中设置「竖井分组 ID」（`shaftGroupId`）。具有相同竖井分组 ID 的区域会在不同楼层之间自动建立 TwoWayFlow 垂直连接。

> **提示**：多楼层模型中如果没有任何竖井连通，系统会在仿真前显示验证警告。

### 5.5 注意事项

- 新楼层为空白画布，不会复制下层几何（可使用「复制楼层」按钮或导入底图作为描图参考）
...
**链式绘制**：确认一面墙后，系统自动以该墙终点为起点开始下一面墙的绘制，无需重新点击起点。按 `Escape` 结束链式绘制。

**正交约束**：墙壁自动锁定为水平或垂直方向（取决于鼠标偏移方向）。

**顶点吸附**：当鼠标靠近已有顶点（12 像素范围内）时自动吸附，确保墙壁精确连接。顶点吸附优先级高于正交约束，以便精确闭合房间。
...
最小尺寸：0.3m × 0.3m。矩形创建为原子操作，撤销时一步撤销全部 4 面墙。
...
切换到删除模式后，鼠标悬停在墙壁或组件上时显示红色高亮预览，点击即可删除。删除墙壁会触发房间重新检测。

也可以选中元素后按 `Delete` 键删除。

### 6.5 悬停预览与光标样式

不同工具模式下，画布光标会自动切换以提示当前操作：
...
在门/窗/删除模式下，鼠标悬停在墙壁或区域上时会显示高亮预览，帮助确认操作目标。
...
**方式二：工具栏右键菜单**（支持全部 10 种类型）
- 右键点击垂直工具栏的「门」按钮 → 弹出组件类型选择菜单
- 选择类型后，工具自动切换为该类型的放置模式
- 工具栏按钮图标和提示文字会更新为当前选中的类型

**方式三：画布右键菜单**
- 右键点击墙壁 →「在墙上放置」→ 选择组件类型
- 组件放置在点击位置（alpha 0.05~0.95 范围内）
...
| | 开口 (opening) | 大开口双向流模型（无门扇），适用于通风口、过道等 |
...
### 8.1 门 / 开口 / 大开口双向流 (TwoWayFlow)

适用于门、大窗、开口、过道等大面积开口。门和开口均使用 TwoWayFlow 模型，区别在于门带有门扇图标，开口为无门扇的通风口。
...
| Cd | 流量系数 | 0.65 |
| area | 开口面积 (m²) | 1.8 |
| openingHeight | 开口高度 (m)，用于双向流分层计算 | 2.0 |
...
### 8.9 二次元件 (QuadraticElement)

$$\dot{m} = \rho \cdot C \cdot \sqrt{|\Delta P|} \cdot \text{sign}(\Delta P)$$

| 参数 | 说明 | 默认值 |
|------|------|--------|
| C | 流动系数 | 0.001 |

### 8.10 防回流阻尼器 (BackdraftDamper)

仅允许正向流动，反向时完全关闭。正向流量计算同幂律孔口。
...
### 8.11 供应扩散器 (SupplyDiffuser) / 回风栅栏 (ReturnGrille)

低阻力末端装置，用于 HVAC 系统的送风和回风口。
...
| 名称 | 房间名称（如「办公室」） | 是 |
| 温度 | 室内温度，显示 °C，存储 K | 是 |
| 体积 | 房间体积 (m³)，为 0 时自动按面积×层高计算 | 是 |
| 面积 | 自动计算的地面面积 (m²) | 否 |
| 顶点数 | 围合多边形的顶点数量 | 否 |
| Zone ID | 内部标识符 | 否 |
| 颜色 | 区域填充色（自动分配） | 否 |
| 竖井分组 | 竖井分组 ID，相同 ID 的区域跨楼层自动连通 | 是 |
| 初始浓度 | 各污染物的初始浓度 (kg/m³)，用于瞬态仿真 | 是 |
...
| 警告 | 温度超出合理范围 (200~400 K) | 允许仿真，提示检查 |
| 警告 | 源引用了不存在的物种或区域 | 允许仿真，忽略无效源 |
| 警告 | 多楼层模型无竖井连通 | 允许仿真，提示可能缺少垂直气流路径 |
| 警告 | 孤立墙上放置了组件 | 允许仿真，提示组件无法建立有效连接 |

浏览器模式下仿真使用模拟数据，系统会显示 toast 提示。完整 C++ 引擎求解需要 Tauri 桌面模式。仿真超时保护为 60 秒。
...
- **保存** (TopBar 右侧)：导出为 `.contam.json` 文件
  - Tauri 模式：原生文件对话框（@tauri-apps/plugin-dialog）
  - 浏览器模式：自动下载
- **打开** (TopBar 右侧)：导入 `.contam.json` 或 `.json` 文件
- **清空**：确认对话框后清除所有数据（不可撤销）
- **暗色模式**：TopBar 月亮/太阳图标切换，偏好自动保存到 localStorage，刷新后保持
...
| 快捷键 | 功能 |
|--------|------|
| `Space`（按住） | 临时平移模式（光标变为抓手） |
| 鼠标滚轮 | 缩放（以光标为中心，范围 5~500 px/m） |
| 左键拖拽空白区域 | 平移画布 |
| 中键拖拽 | 平移画布 |
| 右键 | 上下文菜单（优先级：组件 > 墙壁 > 区域） |

### 18.3 编辑操作

| 快捷键 | 功能 |
|--------|------|
| `Delete` 或 `Backspace` | 删除选中的组件或墙壁 |
| `Escape` | 取消当前操作（墙壁/矩形绘制）/ 清除选择 |
| `Ctrl+Z` | 撤销 |
| `Ctrl+Shift+Z` | 重做 |
...
| 限制 | 说明 |
|------|------|
| 楼层重命名 | `renameStory` 功能已实现，但画布 UI 中尚未提供重命名入口 |
| 高级过滤器画布图标 | SimpleGaseousFilter、UVGI、SuperFilter 等高级过滤器类型在画布上使用通用过滤器图标 |
...
验证文件位于 `engine/test/` 目录，共 266 GoogleTest 用例（16 个测试文件）。前端共 266 Vitest 用例（8 个测试文件）。

### 20.3 后续计划

- 楼层重命名 UI 入口（功能已实现，缺 UI 按钮）
- 示例模型库
- 用户引导教程
- 国际化（中英文切换）
- CFD 区域耦合接口
