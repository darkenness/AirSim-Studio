#include "elements/SimpleParticleFilter.h"
#include <cmath>
#include <stdexcept>
#include <algorithm>
...
SimpleParticleFilter::SimpleParticleFilter(double C, double n,
                                           const std::vector<EfficiencyPoint>& efficiencyTable)
    : C_(C), n_(n), table_(efficiencyTable)
{
    ...
    if (table_.size() < 2) throw std::invalid_argument("Efficiency table needs at least 2 points");
...
FlowResult SimpleParticleFilter::calculate(double deltaP, double density) const {
    FlowResult result;
...
double SimpleParticleFilter::getEfficiency(double diameter_um) const {
    if (table_.empty()) return 0.0;
    if (table_.size() == 1) return table_[0].efficiency;

    // Clamp to table range
    if (diameter_um <= table_.front().diameter) return table_.front().efficiency;
    if (diameter_um >= table_.back().diameter) return table_.back().efficiency;

    // Find interval
    size_t i = 0;
    for (i = 0; i < table_.size() - 1; ++i) {
        if (diameter_um >= table_[i].diameter && diameter_um <= table_[i + 1].diameter) break;
    }

    // Cubic spline evaluation
    double dx = diameter_um - table_[i].diameter;
    double val = splineA_[i] + splineB_[i] * dx + splineC_[i] * dx * dx + splineD_[i] * dx * dx * dx;
    return std::max(0.0, std::min(1.0, val));
}

void SimpleParticleFilter::buildSpline() {
    ...
    if (n == 2) {
        double h = table_[1].diameter - table_[0].diameter;
    ...
    for (size_t i = 0; i < n - 1; ++i)
        h[i] = table_[i + 1].diameter - table_[i].diameter;
    ...
    for (size_t i = 1; i < n - 1; ++i) {
        l[i] = 2.0 * (table_[i + 1].diameter - table_[i - 1].diameter) - h[i - 1] * mu[i - 1];
        mu[i] = h[i] / l[i];
...
std::unique_ptr<FlowElement> SimpleParticleFilter::clone() const {
    return std::make_unique<SimpleParticleFilter>(*this);
}
