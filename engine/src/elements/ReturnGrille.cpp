#include "elements/ReturnGrille.h"
#include <cmath>
#include <stdexcept>

namespace contam {

ReturnGrille::ReturnGrille(double C, double n)
    : C_(C), n_(n)
{
...
FlowResult ReturnGrille::calculate(double deltaP, double density) const {
    FlowResult result;
...
std::unique_ptr<FlowElement> ReturnGrille::clone() const {
    return std::make_unique<ReturnGrille>(*this);
}
